...
function FaqForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2.5 rounded-xl border border-ink/[.08] bg-ink/[.015] p-3.5">
      <input
        type="text"
        value={question}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="rounded-lg px-3.5 py-2 text-xs font-semibold text-foreground/60 hover:bg-ink/[.04]"
        >
          Cancel
        </button>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        {!adding && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-ink/[.04] px-3 py-2 text-xs font-semibold text-foreground/70 hover:bg-ink/[.07]"
          >
        ...
        {faqs.length === 0 && !adding ? (
          ...
          faqs.map((f) =>
            editingId === f.id ? (
              ...
              <div key={f.id} className="flex items-start justify-between gap-3 rounded-xl bg-ink/[.02] p-3.5">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  ...
                  <button
                    ...
                    className="flex h-7 w-7 items-center justify-center rounded-lg text-foreground/40 hover:bg-red-50 dark:bg-red-500/15 hover:text-red-600 dark:text-red-400 disabled:opacity-50"
                  >
                    {deletingId === f.id ? (
