  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-2xl border border-indigo-100 dark:border-indigo-500/25 bg-indigo-50 dark:bg-indigo-500/15/50 p-4">
        <p className="flex items-center gap-1.5 text-sm font-bold text-[#45157b]">
        ...
        <textarea
          ...
          className="mt-3 w-full rounded-lg border border-ink/[.12] bg-surface px-3.5 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-[#45157b]/30"
        />
        <button
          type="button"
          disabled={!quickPrompt.trim()}
          onClick={() => onQuickGenerate(quickPrompt.trim())}
          className="mt-3 flex items-center gap-2 rounded-xl shine-btn-gold relative overflow-hidden bg-[#45157b] px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:opacity-90 disabled:opacity-40"
        >
      ...
      <button
        ...
        onClick={onNext}
        disabled={!template}
        className="mt-1 w-full rounded-xl shine-btn-gold relative overflow-hidden bg-[#45157b] py-3 text-sm font-semibold text-white shadow-sm hover:opacity-90 disabled:opacity-40"
      >
        Continue
      </button>
    </div>
