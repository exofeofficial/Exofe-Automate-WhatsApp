    ...
    <div className="flex flex-col gap-5">
      <div className="rounded-2xl border border-indigo-100 bg-indigo-50/50 p-4">
        ...
        <textarea
          ...
          className="mt-3 w-full rounded-lg border border-black/[.12] bg-white px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#5B4FE9]/30"
        />
        <button
          type="button"
          disabled={!quickPrompt.trim()}
          onClick={() => onQuickGenerate(quickPrompt.trim())}
          className="mt-3 flex items-center gap-2 rounded-xl bg-gradient-to-br from-[#5B4FE9] to-[#7C6FF5] px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:opacity-90 disabled:opacity-40"
        >
      ...
      <button
        ...
        onClick={onNext}
        disabled={!template}
        className="mt-1 w-full rounded-xl bg-gradient-to-br from-[#5B4FE9] to-[#7C6FF5] py-3 text-sm font-semibold text-white shadow-sm hover:opacity-90 disabled:opacity-40"
      >
        Continue
      </button>
    </div>
