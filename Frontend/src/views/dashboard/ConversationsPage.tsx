    ...
    <div className={`flex ${isCustomer ? "justify-start" : "justify-end"}`}>
      <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm ${
        isCustomer
          ...
          : message.from === "ai"
          ? "rounded-br-sm bg-indigo-50 text-foreground"
...
export default function ConversationsPage() {
  ...
  if (conversations.length === 0) {
    return (
      <div className="flex h-[calc(100vh-8rem)] flex-col items-center justify-center gap-3 rounded-2xl border border-black/[.06] bg-white p-6 text-center shadow-sm">
        ...
        {whatsappConnected ? (
          ...
          <>
            ...
            <Link
              ...
              className="mt-2 rounded-xl bg-gradient-to-br from-[#5B4FE9] to-[#7C6FF5] px-5 py-2.5 text-xs font-semibold text-white shadow-sm hover:opacity-90"
            >
              Connect WhatsApp
            </Link>
          </>
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] overflow-hidden rounded-2xl border border-black/[.06] bg-white shadow-sm">
      {/* conversation list */}
      <div className={`flex w-full flex-col border-r border-black/[.06] sm:w-80 sm:shrink-0 ${active ? "hidden sm:flex" : "flex"}`}>
        <div className="border-b border-black/[.06] p-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-full border border-black/[.08] bg-zinc-50 py-2 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#5B4FE9]/25"
            />
          </div>
            ...
            {FILTERS.map((f) => (
              <button
                ...
                className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
                  filter === f ? "bg-[#5B4FE9] text-white" : "bg-black/[.04] text-foreground/60 hover:bg-black/[.07]"
                }`}
              >
                {f}
              </button>
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.map((c) => (
            <button
              ...
              className={`flex w-full items-center gap-3 border-b border-black/[.04] px-4 py-3 text-left transition-colors ${
                activeId === c.id ? "bg-indigo-50" : "hover:bg-black/[.02]"
              }`}
            >
      ...
      {active ? (
        <div className={`flex min-w-0 flex-1 flex-col ${active ? "flex" : "hidden sm:flex"}`}>
          <div className="flex items-center justify-between gap-3 border-b border-black/[.06] p-3 sm:p-4">
            ...
            <motion.button
              ...
              className={`flex shrink-0 items-center gap-1.5 rounded-full px-3.5 py-2 text-xs font-semibold transition-colors ${
                active.mode === "ai"
                  ...
                  : "border border-black/[.1] text-foreground/70 hover:bg-black/[.03]"
              }`}
            >
          ...
          <div className="flex items-center gap-2 border-t border-black/[.06] p-3">
            <input
              type="text"
              value={draft}
              ...
              className="flex-1 rounded-full border border-black/[.1] bg-zinc-50 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#5B4FE9]/25"
            />
            <motion.button
              type="button"
              onClick={handleSend}
