      ...
      <div className="flex h-[calc(100vh-10rem)] flex-col items-center justify-center gap-3 rounded-2xl border border-ink/[.06] bg-surface p-6 text-center shadow-sm">
        ...
        <div className="mt-2 flex gap-3">
          ...
          <Link
            ...
            className="rounded-xl shine-btn-gold relative overflow-hidden bg-[#45157b] px-5 py-2.5 text-xs font-semibold text-white shadow-sm hover:opacity-90"
          >
            Add product
          </Link>
        </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      {selected.size > 0 ? (
        <div className="flex items-center justify-between gap-3 rounded-2xl border border-indigo-100 dark:border-indigo-500/25 bg-indigo-50 dark:bg-indigo-500/15/60 px-4 py-3">
          <p className="text-xs font-semibold text-[#45157b]">{selected.size} selected</p>
          <button
            type="button"
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-[180px] flex-1">
            ...
            <input
              ...
              className="w-full rounded-full border border-ink/[.08] bg-surface py-2 pl-9 pr-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-[#45157b]/25"
            />
          </div>
          <select
            value={categoryFilter}
            onChange={(e) => setCategoryFilter(e.target.value)}
            className="rounded-full border border-ink/[.08] bg-surface px-3 py-2 text-xs font-medium text-foreground/70 focus:outline-none focus:ring-2 focus:ring-[#45157b]/25"
          >
            ...
            {STATUS_FILTERS.map((f) => (
              <button
                ...
                className={`shrink-0 rounded-full px-3 py-2 text-xs font-medium transition-colors ${
                  statusFilter === f ? "bg-[#45157b] text-white" : "bg-ink/[.04] text-foreground/60 hover:bg-ink/[.07]"
                }`}
              >
                {f}
              </button>
        ...
        <div className="min-w-[720px]">
          <div className="flex items-center gap-4 border-b border-ink/[.06] px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-foreground/40">
            <input
              ...
              className="h-4 w-4 shrink-0 rounded border-ink/[.2] text-[#45157b] focus:ring-[#45157b]/30"
            />
